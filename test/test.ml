...
let one       = Node.of_int 1
let two       = Node.of_int 2
let three     = Node.of_int 3
let forty_two = Node.of_int 42
let pi        = Node.of_float 3.1415
...
let () = assert (Parser.parse_datum "foo" = Some (Node.Id (Symbol.of_string "foo")))

let () = assert (Parser.parse_datum "42" = Some forty_two)

let () = assert (Parser.parse_datum "1.23" = Some (Node.of_float 1.23))
...
module Analysis = Drylang.Analysis

let dry input =
  match Drylang.Parser.parse_datum input with
  | None -> assert false
  | Some syntax -> Analysis.analyze_node syntax

let () = assert (Analysis.analyze_node forty_two = Node.Literal (Datum.of_int 42))
